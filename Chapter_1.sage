    ...
    while True:
        ...
        if len(orders) == 6:
            if not time_it:
                ...
                print('          #𝐸ₐ,₃ = {0:>78}  #  y² = x³ + {1:>10} {2}'.format(EC[3][1], EC[3][0], ' :  prime' if is_prime(EC[3][1]) else ''))
...
def find_ec_orders_BM(p, seed_z=1234567890, random_a=True, factorize_orders=False, time_it=False, debug=False):
    ...
    if factorize_orders:
        ...
        if debug:
            ...
            print('  #𝐸ₐ,₃ factors = {0:>100}  # DEC'.format(f'{E_a_factors[3]}'))
            print('  #𝐸ₐ,₄ factors = {0:>100}  # DEC'.format(f'{E_a_factors[4]}'))
            print('  #𝐸ₐ,₅ factors = {0:>100}  # DEC\n'.format(f'{E_a_factors[5]}'))
